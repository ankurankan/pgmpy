...
class TreeSearch(StructureEstimator):
    ...
    def estimate(
        ...
        edge_weights: str or function (default: mutual info)
        ...
        if estimator_type == "chow-liu":
            return TreeSearch.chow_liu(
                self.data, self.root_node, edge_weights, self.n_jobs, show_progress
            )
        ...
        # Step 3: If estimator_type = "tan":
            ...
            D = TreeSearch.chow_liu(
                df_features, self.root_node, edge_weights, self.n_jobs, show_progress
            )
    ...
    @staticmethod
    def chow_liu(
        data, root_node, edge_weights="mutual_info", n_jobs=-1, show_progress=True
    ):
        """
        ...
        if edge_weights == "mutual_info":
            edge_weights = mutual_info_score
        elif not isinstance(edge_weights, callable):
            raise ValueError(
                "edge_weights either needs to be 'mutual_info' or a function of form fun(array, array)"
            )
        ...
        vals = Parallel(n_jobs=n_jobs, prefer="threads")(
            delayed(edge_weights)(data.loc[:, u], data.loc[:, v]) for u, v in pbar
        )
